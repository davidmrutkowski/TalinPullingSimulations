...
Simulation::Simulation()
{
    ...
    retrogradeFlowSpeed = 0.1;
    
    attachedToActin = true;
    attachedToIntegrin = true;
    ...
    // talin break force in pN
    talinActinBreakForce = 10.0;
...
void Simulation::initDerivedValues()
{
    ...
    //zeta (drag coefficient) for spheres
    zeta = 6*pi*eta*beadRadius;
...
void Simulation::readParameterFile(std::string fileName)
{
    ...
    double boxLengths[] = {10.0, 10.0, 10.0};
    ...
    if (inputfile.is_open())
	{
		...
		{	
            ...
            {
                // fix for if text file was created in an windows operating system and run in linux
            ...
            //cout << line.substr(line.size()-1, line.size()) << endl;
            //exit(0);
            ...
            else if(category.compare("springConstantK") == 0)
            {
            ...
            else if(category.compare("retrogradeFlowSpeed") == 0)
            {
                retrogradeFlowSpeed = std::stod(value);
            }
            else if(category.compare("beadRadius") == 0)
            {
                beadRadius = std::stod(value);
            }
            else if(category.compare("numBeadsTalin") == 0)
            {
                numBeadsTalin = std::stoi(value);
            }
            else if(category.compare("attachedToActin") == 0)
            {
                istringstream(value) >> std::boolalpha >> attachedToActin;
            }
            else if(category.compare("attachedToIntegrin") == 0)
            {
                istringstream(value) >> std::boolalpha >> attachedToIntegrin;
            }
            else if(category.compare("eqBondLength") == 0)
            {
            ...
            else if(category.compare("talinActinBreakForce") == 0)
            {
                talinActinBreakForce = std::stod(value);
            }
            else
            {
    ...
    particleGrid.setBinSize(boxLengths[0]*0.3);
    ...
    /*// original talin chain bonds
    bondTypes.emplace_back(SpringBondInfo(filamentBondK, L_zero));
    
    // weakened talin chain bonds
    bondTypes.emplace_back(SpringBondInfo(filamentBondK*0.1, L_zero*1.25));
    
    // integrin-talin linker
    bondTypes.emplace_back(SpringBondInfo(filamentBondK, L_zero));
    
    // talin-actin linker
    bondTypes.emplace_back(SpringBondInfo(filamentBondK, L_zero));*/
    
    //updated bond definitions due to entropic bond
    
    // original talin chain bonds
    bondTypes.push_back(std::unique_ptr<BondInfo> (new SpringBondInfo(filamentBondK, L_zero)));
    
    // average number of amino acids for talin
    int talinN = 145;
    // Kuhn length in um for a typical amino acid length, from Horan et al.
    double aminoAcidB = 0.38 / 1000.0;
    // weakened talin chain bonds
    double tempTemperature = temperature;
    if(thermalForcesOn == false)
    {
        tempTemperature = 300.0;
    }
    
    // tain-talin unfolded bonds
    bondTypes.push_back(std::unique_ptr<BondInfo> (new EntropicSpringBondInfo(boltzmannConstant*tempTemperature, talinN, aminoAcidB)));
    //bondTypes.push_back(std::unique_ptr<BondInfo> (new SpringBondInfo(1000.0, L_zero)));
...
void Simulation::moveParticles()
{
    ...
    for(int i = 0; i < numParticles; i++)
    {
        ...
        else if(filamentTag == -1)
        {
            // this is the integrin particle, dont move it
            
        }
        else if(filamentTag == -2)
        {
            // this is the actin particle, move it with specified velocity
            nextPosition.x = currPosition.x + retrogradeFlowSpeed * dt;
            nextPosition.y = currPosition.y;
...
void Simulation::calcFilamentForces(int f)
{
    ...
    for(int p = 0; p < numTags-1; p++)
    {
        ...
        bool calcBendingForce = false;
        if(calcBendingForce && p > 0)
        {   
            // calc bending forces
...
double Simulation::calcForces()
{
            ...
            if(currTag < tempBondList[b].j)
            {
                ...
                if(firstBond.bondTypeIndex >= 0)
                {
                    ...
                    else
                    {
                        ...
                        //int filamentTag = pinfo.getBeadToFilamentByIndex(p);
                        double bondForceMag = bondForceVector.getMagnitude();
                        double integrinBondForceRate = 0.13*exp(0.16 * bondForceMag);
                        //double talinBondForceRate = 0.018*exp(1.39 * bondForceMag);
                        
                        
                        double talinIntegrinBreakRandom = 0.0;
                        do
                          {
                            talinIntegrinBreakRandom = dis(threadEngine);
                          }
                        while(talinIntegrinBreakRandom == 0.0);
                        double talinTalinUnfoldRandom = 0.0;
                        do
                          {
                            talinTalinUnfoldRandom = dis(threadEngine);
                          }
                        while(talinTalinUnfoldRandom == 0.0);

                        if(firstBond.bondTypeIndex == 0)
                        {
                            double talinBondForceRate = unfolding_rate_constants[currTag - 1]*exp(unfolding_alpha[currTag-1] * bondForceMag);
                            
                            //if(talinBondForceRate * dt > talinTalinUnfoldRandom)
                            if(1.0 - exp(-talinBondForceRate * dt) > talinTalinUnfoldRandom)    
                            {
                                if(currTag == 8)
                                {
                                    // only unfold domain 8 if domain 7 unfolds
                                }
                                else
                                {
                                    pinfo.setBondType(tempBondList[b].bondTag, 1);
                                    ...
                                    if(currTag == 7)
                                    {
                                        pinfo.setBondType(tempBondList[8].bondTag, 1);
                                    }
                                }
                            }
                        }
                        else if(firstBond.bondTypeIndex == 2)
                        {
                          //if(integrinBondForceRate * dt > talinIntegrinBreakRandom)
                          if(1.0 - exp(-integrinBondForceRate * dt) > talinTalinUnfoldRandom)     
                            {
                              #pragma omp critical
                              {
                                removalList.push_back(tempBondList[b].bondTag);
                        ...
                        else if(firstBond.bondTypeIndex == 3)
                        {
                        }
    ...
    for(int p = 0; p < pinfo.getNumParticles(); p++)
    {
        ...
        // boundary force for y-direction of 10 pN, both upper and lower boundaries
        if(fabs(currPos.y) >= simBoxYMax)
        {
            pinfo.addForceByIndex(p, 10.0 * Coordinate {0.0, -1.0, 0.0} * copysign(1.0, currPos.y));
        }
...
void Simulation::writeFRCFile(ofstream& outputFRC)
{
  int numBonds = pinfo.getNumBonds();
  for(int p = 0; p < numBonds; p++)
    {
      std::vector <struct HalfBond> tempBondList = pinfo.getBondTagsByIndex(p);
      for(int b = 0; b < tempBondList.size(); b++)
	{
	  Bond currBond = pinfo.getBond(tempBondList[b].bondTag);
	  if(currBond.bondTypeIndex == 2 && b==0)
	    {
	      Bond tempBond = pinfo.getBond(b);
	      Coordinate currBondForce = getBondForce(tempBond);
	      outputFRC << " " << currSimTime << " " << currBondForce.x << " " << currBondForce.y << " " << currBondForce.z << endl;
	    }
	}
    }
}

void Simulation::writeFTAFile(ofstream& outputFTA)
{
  int numBonds = pinfo.getNumBonds();
    for(int b = numBonds-1; b > -1; b--)
    {
        Bond currBond = pinfo.getBondByIndex(b);

        if(currBond.bondTypeIndex == 3)
        {
          Coordinate currBondForce = getBondForce(currBond);
          outputFTA << " " << currSimTime << " " << currBondForce.x << " " << currBondForce.y << " " << currBondForce.z << endl;
        }
    }
}

void Simulation::writeEndingFile(std::ofstream& outputEND)
{
    int numParticles = pinfo.getNumParticles();
    
    Coordinate posFirstTalin = pinfo.getPos(1);
    Coordinate posLastTalin = pinfo.getPos(numParticles-2);
    
    Coordinate disp = simBox.calcDisplacement(posFirstTalin, posLastTalin);
    
    outputEND << currSimTime << " " << retrogradeFlowSpeed*currSimTime << " " << disp.getMagnitude() << endl;
}
...
{   
    ...
    double bondEqLength = bondTypes[0]->getEqDist();
    
    for(int f = 0; f < numBeads; f++)
    {
        // should put new particle in grid and update neighbors of all associated particles
        Coordinate newPosition = {startPos.x + direction.x*bondEqLength*f, startPos.y + direction.y*bondEqLength*f, startPos.z + direction.z*bondEqLength*f};
        ...
        if(filamentBondsInBondList && priorTag > -1)
        {
            ...
            newBond.destructionTime = currSimTime + 10000000.0;
...
void Simulation::removeBondByIndex(int bondIndex)
{
   ...
   // cout << "removeBondByIndex: " << filamentI << " " << filamentJ << endl;
    if(filamentI >= 0 && filamentJ >= 0 && filamentI == filamentJ)
    {
...
void Simulation::run()
{
    ...
    ofstream outputFRC;
    outputFRC.open ("out-" + outputFileName + ".frc");
    
    ofstream outputFTA;
    outputFTA.open ("out-" + outputFileName + ".fta");
    
    ofstream outputEND;
    outputEND.open ("out-" + outputFileName + ".end");
    ...
    for(int mainLoopVar = 0; mainLoopVar < numSteps; mainLoopVar++)
    {
        ...
        {
            // integrin bead
            Particle integrinParticle(0.0, -simBox.getBoxLength(1)*0.5, 0.0);
            integrinParticle.setType(-1);
            int numStressMeasurements = 2;
            int newTag = pinfo.addParticle(integrinParticle, numStressMeasurements);
            
            // talin filament
            addFilament(numBeadsTalin, Coordinate {0.0, pinfo.getPosByIndex(0).y+L_zero, 0.0}, Coordinate {0.0, 1.0, 0.0}, false);
            
            // actin bead
            double actinBeadY = pinfo.getPosByIndex(pinfo.getNumParticles()-1).y+L_zero;
            if(actinBeadY > simBox.getBoxLength(1)*0.5)
            {
                cout << "Talin chain is partially outside of box, excluded volume may not be calculated correctly!: " << actinBeadY << " > " << simBox.getBoxLength(1)*0.5 << endl;
            }
            Particle actinParticle(0.0, pinfo.getPosByIndex(0).y+L_zero*(numBeadsTalin+1), 0.0);
            actinParticle.setType(-2);
            newTag = pinfo.addParticle(actinParticle, numStressMeasurements);
            
            // bond between end of talin and integrin
            if(attachedToIntegrin)
            {
                Bond newBond = {0, 1, 2};
                newBond.creationTime = currSimTime;                    
                newBond.destructionTime = currSimTime + 10000.0;
                        
                pinfo.addBond(newBond);
            }
            
            // bond between other end of talin and actin
            if(attachedToActin)
            {
                Bond newBond = {pinfo.getNumParticles()-2, pinfo.getNumParticles()-1, 3};
                newBond.creationTime = currSimTime;                    
                newBond.destructionTime = currSimTime + 10000.0;
                pinfo.addBond(newBond);
            }
            ...
            {
                Coordinate currPos = pinfo.getPosByIndex(p);
                cout << pinfo.getBeadToFilamentByIndex(p) << " " << currPos.x << " " << currPos.y << " " << currPos.z << endl;
            }*/
        }
        ...
        {
            writeEndingFile(outputEND);
            
            outputFRC << std::endl;
            writeFRCFile(outputFRC);
            
            outputFTA << std::endl;
            writeFTAFile(outputFTA);
            
            // stop main loop after break at talin-integrin
            break;
        }
